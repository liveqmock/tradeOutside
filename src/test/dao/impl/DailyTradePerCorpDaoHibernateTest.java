...
import com.github.cruiser.tradeoutside.dao.impl.DailyTradePerCorpDaoHibernate;
...
public class DailyTradePerCorpDaoHibernateTest {
	...
	public static void main(String[] args) {
		...
		DailyTradePerCorpDao commonDao
			= ctx.getBean("dailyTradePerCorp", DailyTradePerCorpDaoHibernate.class);

		/*List corps = commonDao.findAll();
		Iterator it = corps.iterator();
		
		while(it.hasNext()){
			Object oldCorp = it.next();
			System.out.println("删除"+oldCorp);
			//oldCorp.getDccTerminals().add("000000");
			//commonDao.update(oldCorp);
			//corpDao.delete(oldCorp);

		}*/

		commonDao.save(dailyTrade);

		/*corps = commonDao.findAll();
		it = corps.iterator();
